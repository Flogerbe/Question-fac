...
- **Vote configurable** — vote pour le maillot ou tout autre sujet (intitulé modifiable)
...
- Gestion des votes (maillot ou autre)
...
### Méthode 1 — Installateur web (recommandée)

C'est la méthode la plus simple, sans ligne de commande complexe.
...
# 3. Préparer l'environnement (ne pas configurer la BDD pour l'instant)
cp .env.example .env
php artisan key:generate
```

Puis ouvrir **`http://votre-site.fr/install`** dans le navigateur.

L'installateur se charge de :
- Tester la connexion à la base de données
- Écrire les paramètres dans `.env`
- Créer toutes les tables (`migrate`)
- Pré-remplir les données initiales (`db:seed`)
- Créer le compte administrateur
- Verrouiller la page `/install` (inaccessible après installation)

> **Note :** La page `/install` redirige automatiquement vers l'accueil si le projet est déjà installé.
...
Lorsque vous déployez une nouvelle version du projet sur le serveur :
...
| Intitulé du vote | Texte du bouton de vote (ex : "Vote Maillot", "Vote Logo"…) |
| Visibilité du vote | Afficher ou masquer la section vote sur le site public |
...
3. Les résultats sont sauvegardés, relançables à tout moment, et affichés sur la page d'accueil publique
...
app/
├── Http/Controllers/
│   ├── InstallController.php     # Installateur web (/install)
│   ├── QuizController.php        # Logique du quiz (jouer, jokers, vérifier)
│   ├── HomeController.php        # Page d'accueil + classement
│   ├── JerseyController.php      # Vote configurable
│   └── Admin/
│       ├── DashboardController.php  # Dashboard + migrate()
